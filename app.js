...
const client = new TwilioClient(); //It will automatically fill env variables in here
...
app.get("/api/compliments", async (req, res) => {
  ...
  const sentMessages = await client.messages.list({ from: twilioPhoneNumber }); // done
  // TODO: Gather only the body of those messages for sending to the client
  const compliments = sentMessages.map(message => message.body); // done
